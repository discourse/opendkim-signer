...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
	...
	&& DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y opendkim ruby socat \
...
COPY opendkim.conf.erb /etc/
COPY init /sbin/
COPY build-dkim-config /usr/local/sbin/
